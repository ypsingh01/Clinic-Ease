...
async function main() {
  ...
  if (seedDemo) {
    ...
    // Minimal staff placeholders — admin creates patients in production
    patient = admin
    doctorUser = admin
  }
  ...
  if (!seedDemo) {
    console.log('Seeded clinic catalog + bootstrap admin', { admin: admin.email })
    return
  }
  ...
  console.log('Seeded ClinicEase DB (demo users)')
